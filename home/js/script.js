...
// Función para manejar el modal del dashboard
function initDashboardModal() {
    const dashboardLink = document.querySelector('.dashboard-link');
    const authModal = document.getElementById('authModal');
    const closeModal = document.querySelector('.close-modal');

    if (dashboardLink && authModal) {
        // Abrir modal
        dashboardLink.addEventListener('click', (e) => {
            e.preventDefault();
            console.log('Dashboard clicked'); // Para debug
...
function showRemediation(alertData) {
    const remediationModal = document.createElement('div');
    remediationModal.className = 'modal remediation-modal';
    
    remediationModal.innerHTML = `
        <div class="modal-content">
            <div class="modal-header">
                <h2>Plan de Remediación</h2>
            ...
            <div class="modal-body">
                <div class="alert-details">
                    ...
                    <div class="alert-info">
                        <span class="alert-location">
                            <i class="fas fa-map-marker-alt"></i> ${alertData.location}
                        </span>
                        <span class="alert-ip">
                            <i class="fas fa-network-wired"></i> ${alertData.ip}
                        </span>
                    </div>
                </div>
                <div class="remediation-steps">
                    ...
                    <ol>
                        <li>Aislar el sistema afectado</li>
                        <li>Analizar logs del sistema</li>
                        <li>Aplicar parches de seguridad</li>
                        <li>Verificar la integridad del sistema</li>
                        <li>Monitorizar actividad sospechosa</li>
                    </ol>
                </div>
    ...
    remediationModal.style.display = 'block';

    // Cerrar modal
    const closeBtn = remediationModal.querySelector('.close-modal');
    closeBtn.onclick = () => {
        remediationModal.remove();
    };

    // Cerrar al hacer clic fuera
    remediationModal.onclick = (e) => {
        if (e.target === remediationModal) {
            remediationModal.remove();
        }
    };
}

// Añadir el evento a los botones de remediar
document.addEventListener('click', (e) => {
    if (e.target.matches('.remediar-btn')) {
        e.preventDefault();
        ...
        const alertData = {
            ...
            location: alertCard.querySelector('.location').textContent,
            ip: alertCard.querySelector('.ip').textContent
        };
        showRemediation(alertData);
    }
});
...
function previewReport(button) {
    ...
    const reportData = {
        title: reportItem.querySelector('.report-title').textContent,
        date: reportItem.querySelector('.report-date').textContent,
        stats: reportItem.querySelector('.report-stats').textContent
    };
    ...
    reportElement.innerHTML = `
        <div class="report-info">
            ...
            <div class="report-stats">
                <div>Amenazas: ${data.threats}</div>
                <div>Incidentes: ${data.incidents}</div>
            </div>
        </div>
        <div class="report-actions">
            <button onclick="previewReport(this)" class="preview-btn">
                <i class="fas fa-eye"></i>
                Previsualizar
            </button>
            <button onclick="downloadReport(this)" class="download-btn">
                <i class="fas fa-download"></i>
                Descargar
            </button>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    // Manejar clic en el enlace del dashboard
    const dashboardLink = document.querySelector('.dashboard-link');
    const authModal = document.getElementById('authModal');
    const closeModal = document.querySelector('.close-modal');
